...
class RenderCheeseburgerMenuBlock extends ControllerBase {
  ...
  public function renderTree($config, $current_route) {
    $menus = [];
    foreach ($config['menu'] as $id => $data) {
      switch ($data['title']) {
        case '0':
          $title = '';
          $menu = Menu::load($id);
        ...
        case '2':
          $navigation_title = $title = $this->t($data['manual_title']);
          break;
      }
      $menus[] = [
        'tree' => $this->getMenuTree($id),
        'id' => $id,
        'menu_weight' => $data['menu_weight'],
        'title' => $title,
        'collapsable_title' => empty($data['collapsable_title']) ? '' : ' cheeseburger-menu__menu-list-item--expanded',
        'navigation_title' => $navigation_title,
      ];
    }

    $vocabularies = [];
    foreach ($config['vocabulary'] as $id => $data) {
      switch ($data['title']) {
        case '0':
          $title = '';
          $vocabulary = Vocabulary::load($id);
          $navigation_title = $vocabulary->get('name');
          break;

        case '1':
          $vocabulary = Vocabulary::load($id);
          $navigation_title = $title = $vocabulary->get('name');
          break;
      ...
      $vocabularies[] = [
        'tree' => $this->getVocabularyTree($id),
        'id' => $id,
        'menu_weight' => $data['menu_weight'],
        'title' => $title,
        'collapsable_title' => empty($data['collapsable_title']) ? '' : ' cheeseburger-menu__menu-list-item--expanded',
    ...
    $additional_menus = [];
    // Cart show.
    if (array_key_exists('cart', $config) && $config['cart']['show'] == 1) {
      $additional_menus[] = [
        'id' => 'cart',
    ...
    // Phone show.
    if (array_key_exists('phone', $config) && $config['phone']['show'] == 1) {
      if ($config['phone']['store'] == 0) {
        ...
        if ($this->moduleHandler()->moduleExists('commerce_store')) {
          ...
          if (!empty($store) && $store->hasField('field_phone')) {
            if (!empty($store->get('field_phone')->getValue())) {
              $additional_menus[] = [
                'id' => 'phone',
                'menu_weight' => $config['phone']['menu_weight'],
    ...
    if (array_key_exists('lang_switcher', $config) && $config['lang_switcher']['show'] == 1) {
    ...
    $tree = array_merge($menus, $vocabularies, $additional_menus);
    ...
    if (!empty($config['active_state_enable'])) {
   ...
   */
  public function formTree(&$tree, $current_url = FALSE, $active_state = 1) {
    $count_active = 0;
